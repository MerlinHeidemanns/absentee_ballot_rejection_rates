...
model {
  ...
  target += normal_lpdf(y_bar[:,2] | rep_vector(to_vector(beta[:,1])' * to_vector(beta[:,2]), J), sigma[2]);
  target += normal_lpdf(y_bar[:,3] | rep_vector(to_vector(beta[:,2])' * to_vector(beta[:,3]), J), sigma[3]);
}
