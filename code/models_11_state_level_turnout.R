...
ppd_rejected <- ggplot(data = ybar_rejected, aes(x = y, fill = kind)) + 
  ...
  theme_bw() + 
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
ggsave(paste0("plots/m11_", Sys.Date(), "_ppd_rejected_by_state.jpeg"), ppd_rejected)
ppd_requested
ppd_submitted
ppd_rejected
...
ggsave(paste0("plots/m11_", Sys.Date(), "_ppd_minimum.jpeg"), minimum_predicted)
...
ggsave(paste0("plots/m11_", Sys.Date(), "_ppd_maximum.jpeg"), maximum_predicted)
...
res3 <- ggplot(counties_df %>% filter(State != "HI", State != "AK"), aes(long, lat, fill = residuals3)) +
  geom_polygon(aes(group = group)) +
  facet_wrap(~State, scales = "free")
ggsave(paste0("plots/m11_", Sys.Date(), "_residuals_requested.jpeg"), res1)
ggsave(paste0("plots/m11_", Sys.Date(), "_residuals_submitted.jpeg"), res2)
ggsave(paste0("plots/m11_", Sys.Date(), "_residuals_rejected.jpeg"), res3)
...
ggsave(paste0("plots/m11_", Sys.Date(), "_pr_with_turnout_by_state.jpeg"), plt_grid)
...
ggsave(paste0("plots/m11_", Sys.Date(), "_n_rejected_black_by_state_turnout_by_state.jpeg"), plt_n_black_rejected)
## hispanic ----
...
ggsave(paste0("plots/m11_", Sys.Date(), "_n_rejected_latinx_by_state_turnout_by_state.jpeg"), plt_n_latinx_rejected)
## asian ----
...
ggsave(paste0("plots/m11_", Sys.Date(), "_n_asian_latinx_by_state_turnout_by_state.jpeg"), plt_n_asian_rejected)
## white ----
require(scales)
...
ggsave(paste0("plots/m11_", Sys.Date(), "_group_shares_turnout_by_state.jpeg"), plt_rates, width = 14, height = 8)
