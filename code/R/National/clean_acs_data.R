...
################################################################################
## functions
source("code/R/National/functions.R")
################################################################################
## Load data
df_acs_a <- read.csv("data/acs_13_18_2.csv")
df_acs_b <- read.csv("data/acs_econ_13_18.csv")
df_acs_c <- read.csv("data/acs_econ_13_18_full.csv")
df_acs_d <- read.csv("data/acs_econ_pov13-18.csv")
df_outcome <- read.csv("data/election_outcome.csv")
################################################################################
# Clean ACS data
# * This code mainly translates the existing variables in the data into something that's
# usable by organizing the types and names
df_acs <- df_acs_a %>%
  left_join(df_acs_b, by = c("FIPSCode")) %>% 
  left_join(df_acs_c, by = c("FIPSCode")) %>% 
  left_join(df_acs_d, by = c("FIPSCode")) %>%
  left_join(df_outcome, by = c("FIPSCode")) %>%
...
################################################################################
## Manual fixes
# The FIPS code for these two counties changed
# submitted 
