################################################################################
## Fit the ecological regression model to the data
################################################################################
# Notes
...
# * Ecological inference model based on race
# * Look at model fit based on coverage
# * Look at prediction based on model
# * Assess how this changes 
# * 1. voting power of each group
# * 2. support for each party
# * Assess proportions within each group of voters, requesters, submitters, rejected
################################################################################
## Load functions
source("code/R/Nationalload_model_data.R")
...
region_crosswalk <- df_subset %>% 
  ...
  arrange(group_id)
write_rds(df_subset, "model_fits/fit_m11_states_df_subset.Rds")
################################################################################
## Model
# * Fit and write the model to disk
...
# * Read the model
# * Estimate y_bar (shares rejected etc) with some iterations
# * Given unconstrained sampling adjust by assigning to 0 and 1
...
ybar[ybar < 0] <- 0
################################################################################
## PPC: Estimate overlap with real data
# * What share of observations falls within the 50/95% interval?
...
# State-level
# * Compile coverage at the state level
## 95% coverage plot
...
################################################################################
## Look at minimum and maximum predicted
# minimum
...
################################################################################
# Voting power - Estimate how many votes are expected to be rejected by group
beta_region <- rstan::extract(fit, pars = "beta")[[1]]
...
## Simulation
# * Assumes that 70% vote absentee so no rate of requested needed
# * Take voters from previous elections, shares are knowns, then use
# * estimates for submission and rejection rates from model
...
for (id in sample(1:3000, 50, replace = FALSE)){
  ...
  for (i in 1:nrow(df_subset)) {
    ...
    n_limit = floor(n_voters * .7) ## Assume that 70% vote absentee
...
## nation ----
# * Aggregate values nationally
df_sim <- read_rds("model_fits/fit_simulated_70percent_turnout_by_state.Rds")
df_sim <- df_sim %>% group_by(FIPSCode) %>% mutate(sim = sequence(n()))
...
## change in support
# * Given subgroup level tendencies of voting for either party how does this 
# * affect vote shares
df_sim <- read_rds("model_fits/fit_simulated_70percent_turnout_by_state.Rds")
df_sim <- df_sim %>% group_by(FIPSCode) %>% mutate(sim = sequence(n()))
df_sim <- df_sim %>% group_by(State, sim) %>% 
