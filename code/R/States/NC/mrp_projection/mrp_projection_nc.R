...
# MRP estimates
################################################################################
# The code fits an MRP model to 2020 data and poststratifies to the voting 
# population in 2016 and 2020. It starts with the model and then does 2016 and 
# 2018.
################################################################################
...
## Check whether cells are filled
...
fit <- rstanarm::stan_glmer(cbind(rejected, ballots - rejected) ~ 
                            ...
                            cores = 4)
# post stratification table from the previous election
...
round(c(mean_sample_popn_pref, sd_sample_popn_pref), 5)

# Prediction by ethnic group
