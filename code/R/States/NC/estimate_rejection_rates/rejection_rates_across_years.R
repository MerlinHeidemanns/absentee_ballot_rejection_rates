################################################################################
## Compare rejection rates across years
################################################################################
#' Includes
#' income v race v year
#' race v year
#' pid v race v year
#' race income age only for 2016 and 2020
################################################################################
# functions
source("code/R/functions/functions_nc.R")
################################################################################
# text
...
df_2020 <- df
source("code/R/States/NC/data_cleaning/clean_absentee_nc_2018.R")
df_2018 <- df
source("code/R/States/NC/data_cleaning/clean_absentee_nc_2016.R")
df_2016 <- df
################################################################################
## Merge data across years removing bad types
df_2020 <- df_2020 %>% 
...
df_plot <- extract_quantiles_mean(theta, c(2,3), 95, 
                                  ...
                                  c("Democrat",
                                    ...
                                    "Republican"),
                                  c("year", "ethn", "pid"))
# * Plot with facet by pid and race
ggplot(data = df_plot %>%
...
# * Plot with facet by pid and year
ggplot(data = df_plot %>%
         mutate(pid = factor(pid, levels = c("Democrat", "Unaffiliated", "Republican")),
                year = factor(year, levels = c("2016", "2018", "2020")))
...
################################################################################
# race x income x age only for 2016 and 2020
# load data
source("code/States/NC/data_cleaning/clean_absentee_nc_2020.R")
df_2020 <- df
source("code/States/NC/data_cleaning/clean_absentee_nc_2016.R")
df_2016 <- df
# Merge data
df_2020 <- df_2020 %>% 
